...
const RankRow: React.FC<{ team: Team; userTeamId: string | null }> = ({ team, userTeamId }) => {
  const rankColor = RANK_COLOR[team.rank] ?? '#9CA3AF';
  const isTop3 = team.rank <= 3;
  const isUserTeam = userTeamId !== null && userTeamId === team.id;
  const rowBgColor = isUserTeam ? USER_TEAM_CARD_BACKGROUND : TOP3_CARD_BACKGROUND[team.rank] ?? '#fff';

  return (
    <View style={[s.row, isTop3 && s.rowTop3, isUserTeam && s.userTeamRow, { backgroundColor: rowBgColor }]}> 
      <Text style={s.rankNum}> 
        {String(team.rank).padStart(2, '0')}
      </Text>

      <View style={[s.avatarCircle, isTop3 && { borderColor: rankColor, borderWidth: 2 }]}> 
        <Text style={s.avatarLetter}>{team.name.charAt(0).toUpperCase()}</Text>
      </View>
