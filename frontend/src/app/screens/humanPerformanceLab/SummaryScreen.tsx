...
export default function SummaryScreen() {
  ...
  const [attempts, setAttempts] = useState<{ attempt: number; score: number; improvement: number }[]>([]);
  ...
  return (
    <SummaryTemplate activityTitle="Performance Lab" emoji="🏃" totalPoints={totalPoints} reflection={reflection}>
      ...
      {attempts[bestIndex] && (
        <Section title="Best Movement">
          <StatRow
            label="Improvement with feedback"
            value={attempts[bestIndex].improvement > 0 ? `+${attempts[bestIndex].improvement}` : attempts[bestIndex].improvement}
            unit="pts"
            highlight
          />
        </Section>
      ...
      <Section title="Prediction">
        <StatRow label="You predicted" value={prediction != null ? `Movement ${prediction}` : "—"} />
        <StatRow label="Most improved" value={`Movement ${bestIndex + 1}`} />
