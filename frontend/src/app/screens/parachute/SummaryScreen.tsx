...
export default function SummaryScreen() {
  ...
  const [attempts, setAttempts] = useState<{ attempt: number; score: number; time: number; distance: number; mass: number }[]>([]);
        ...
        const rows = await Promise.all(
          session.activitiesCompleted.map(async (a, i) => {
            ...
            return {
              attempt: i + 1,
              score: a.score,
              time: data.experimentData?.time ?? 0,
              distance: data.experimentData?.distance ?? 0,
              mass: data.experimentData?.mass ?? 0,
            };
          })
        );
  ...
  return (
    <SummaryTemplate activityTitle="Parachute Drop" emoji="🪂" totalPoints={totalPoints} reflection={reflection}>
      ...
      {attempts[bestIndex] && (
        <Section title="Best Design Measurements">
          <StatRow label="Fall time" value={attempts[bestIndex].time} unit="s" />
          <StatRow label="Drop distance" value={attempts[bestIndex].distance} unit="m" />
          <StatRow label="Mass" value={attempts[bestIndex].mass} unit="g" />
        </Section>
      )}
      <Section title="Prediction">
        <StatRow label="You predicted" value={prediction != null ? `Design ${prediction}` : "—"} />
        <StatRow label="Best design" value={`Design ${bestIndex + 1}`} />
